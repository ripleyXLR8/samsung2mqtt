...
Pont **local** entre les appareils électroménagers Samsung Wi-Fi (lave-linge,
sèche-linge, four, réfrigérateur…) et un broker **MQTT**, avec découverte façon
Home Assistant — donc consommable tel quel par le plugin **MQTT Discovery** de
Jeedom, comme `liebherr2mqtt` et `electrolux2mqtt`.

Contrairement à l'API cloud SmartThings (payante à partir d'octobre 2026), ce
pont parle **directement à l'appareil sur le réseau local** en OCF/IoTivity
(DTLS), sans passer par le cloud. L'appairage initial dans l'app SmartThings
reste nécessaire une fois, pour que l'appareil fasse confiance au certificat.

Image : `ghcr.io/ripleyxlr8/samsung2mqtt:latest` (multi-arch amd64 + arm64).
Gabarit Unraid : [`ripleyXLR8/unraid-templates`](https://github.com/ripleyXLR8/unraid-templates).

## Origine

Ce dépôt reprend le projet **[QuiteYellow/SmartThings-Local](https://github.com/QuiteYellow/SmartThings-Local)**
de Jack Nagy (licence **MIT**, voir `LICENSE` et `NOTICE`), qui fournit la pile
protocole OCF/DTLS et le pont MQTT (`mqtt_demo/`). L'ajout maison se limite à
l'empaquetage : workflow de build d'image Docker multi-arch vers GHCR et gabarit
Unraid. Le README d'origine est conservé dans `README.upstream.md`.

Projet indépendant, non affilié à Samsung Electronics.
